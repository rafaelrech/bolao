...
public class DBServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (u != null && u.getId() != -1 && !(u.getRole().equalsIgnoreCase("admin"))) {
		...
		if (action.equalsIgnoreCase("LOAD")) {
			...
			if (object.equalsIgnoreCase(BaseDBUtil.USER_BEAN)) {
				BaseDBUtil.loadUsers();
			}
			if (object.equalsIgnoreCase(BaseDBUtil.BOLAO_BEAN)) {
				BaseDBUtil.loadBallots();
			}
			if (object.equalsIgnoreCase(BaseDBUtil.USER_BOLAO_BEAN)) {
	...
	private String getRedirectInfo(String source) {
		// TODO which jsp?
		return WebUtils.getRedirectInfo(source, "db-admin.jsp");
	}
}
