	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (action.equalsIgnoreCase("LOGIN")) {
			...
			if (u != null) {
				...
				if (u.getId() == -1) {
					if (session.getAttribute(BaseSessionUtil.LOGIN_REDIRECT) != null) {
						String redStr = (String) session.getAttribute(BaseSessionUtil.LOGIN_REDIRECT);
						session.setAttribute(BaseSessionUtil.LOGIN_REDIRECT, null);
						response.sendRedirect(redStr);
					} else {
