 ...
 */
@WebServlet("/nba2016-mm/CalculateServlet")
...
System.out.println("****************");
System.out.println("****************");
System.out.println("COMECOU " + games.size() + " GAMES ");
System.out.println("****************");
System.out.println("****************");
		for (Nba2016User user : users) {
			...
			int mvpBetScore = 0;
			if(user.getUserId()==2) {
				System.out.println("BETS : " + user.getGameBets().size());
			}
			...
			for (Nba2016GameBet bet : user.getGameBets()) {
				for (Nba2016Game game : games) {
					...
					if(user.getUserId()==2) {
						System.out.println("GAMES SCORES : " + gameBetScore + "\t GAME ID : " + game.getId());
					}
			...
			ArrayList<UserBolao> ballots = user.getUser().getBallots();
			for (UserBolao userBolao : ballots) {
				if(userBolao.getBolaoId()==2) {
					userBolao.setUserScore(user.getScore());
					UserBolaoDao.getInstance().update(userBolao);
				}
			}
		}
